...
class dataExtractor:
    ...
    def extract(self):
        ...
        with open(os.getcwd() + '\\appconfig.yaml') as app_config_file:
            ...
            for extract_folder in extract_folders['extractFolders']:
                self.extract_set(extract_folder)

        app_config_file.close()

    # process each extract set
    def extract_set(self, extract_folder):
        ...
        try:
            ...
            workbook = self.get_template_workbook(extract_folder, extract_config['templateFilename'])
            ...
            for extract_detail in extract_config['extractDetails']:
                try:
                    # extract value
                    value = self.extract_value(extract_folder, extract_detail)
            ...
            if not error_count:
                # save the workbook
                self.write_template(workbook, extract_folder, extract_config['templateFilename'])
            else:
                print('ERROR count encountered: ' + str(error_count) + ', while processing ' + extract_folder)

    # write template
    def write_template(self, workbook, extract_folder, template_filename):
    ...
    def extract_value(self, extract_folder, extract_detail):
        try:
    ...
    # get the workbook
    def get_template_workbook(self, extract_folder, filename):
    ...
    def set_value_to_template(self, workbook, sheet_num, cell_address, value):
        try:
